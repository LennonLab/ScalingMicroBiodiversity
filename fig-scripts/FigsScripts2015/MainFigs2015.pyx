...
def Fig1():
    ...
    for index, i in enumerate(metrics):
        ...
        for ii in range(len(MicListX)):
            plt.scatter(MacListX[ii], MacListY[ii], color = 'LightCoral', alpha= 1 , s = 4, linewidths=0.5, edgecolor='Crimson')
            plt.scatter(MicListX[ii], MicListY[ii], color = 'SkyBlue', alpha= 1 , s = 4, linewidths=0.5, edgecolor='Steelblue')

            micY.append(MicListY[ii])
            macY.append(MacListY[ii])
            micX.append(MicListX[ii])
            macX.append(MacListX[ii])
        ...
        if index == 0:
            ...
            plt.legend(bbox_to_anchor=(-0.04, 1.2, 2.47, .2), loc=10, ncol=2,
